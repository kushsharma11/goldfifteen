...
def test_market_revisions_are_kept(tmp_path):
    ...
    market = MarketWindow(market_id="test", start_time=t, end_time=t+timedelta(minutes=15), available_at=t, target_price=4300)
    db.add_market(market)
    db.add_market(market.model_copy(update={"result_yes": True, "available_at": t+timedelta(minutes=16), "status": "settled"}))
...
def test_configuration_bounds():
    ...
    assert Settings(_env_file=None).kelly_fraction == .25
