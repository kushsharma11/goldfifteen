...
from gold_model.providers import (
    CSVComexProvider,
        ...
        "parsed": [
            {
                ...
                "price": {
                    ...
                    "conf": "10000000",
                    "expo": -8,
                    "publish_time": int(timestamp.timestamp()),
                },
                "metadata": {"slot": 1},
            }
        ]
...
def market_payload(**kwargs):
    return {
        "ticker": "KXGOLD15M-TEST-00",
        "event_ticker": "KXGOLD15M-TEST",
        ...
        "status": "settled",
        "result": "yes",
        "expiration_value": "4301.25",
        "settlement_value_dollars": "1.0000",
        "settlement_ts": (START + timedelta(minutes=16)).isoformat(),
        **kwargs,
    }
    ...
    async def run():
        async with client_for(pyth_payload()) as client:
            provider = PythProvider(
                settings(pyth_api_key="test", pyth_feed_id=FEED),
                client,
                raw_sink=lambda *args: raw.append(args),
            )
...
def test_pyth_history_never_backdates_future_returned_tick():
    async def run():
        ...
        async with httpx.AsyncClient(transport=httpx.MockTransport(response)) as client:
            provider = PythProvider(
                settings(pyth_api_key="test", pyth_feed_id=FEED, historical_latency_seconds=1),
                client,
            )
            points = await provider.historical_prices(
                START, START + timedelta(seconds=4), step_seconds=1
            )
            assert len(points) == 1
    ...
    async def run():
        async with client_for(pyth_payload(feed="b" * 64)) as client:
            ...
            with pytest.raises(ProviderError, match="requested feed"):
                await PythProvider(
                    settings(pyth_api_key="test", pyth_feed_id=FEED), client
                ).latest_price()
        ...
        def response(request):
            ...
            if request.url.path.endswith("price_feeds"):
                return httpx.Response(
                    200,
                    json=[
                        {"id": FEED, "attributes": {"symbol": "Metal.XAU/USD"}},
                        {"id": "b" * 64, "attributes": {"symbol": "Metal.XAG/USD"}},
                    ],
                )
        ...
        async with client_for(
            {
                "market": market_payload(
                    floor_strike=None,
                    yes_sub_title="Target Price: $4300.10",
                    expiration_value="not available",
                )
            }
        ) as client:
...
def test_kalshi_history_no_assumption_or_settlement_time_never_backdates_label():
    async def run():
        async with client_for(
            {"markets": [market_payload(settlement_ts=None)], "cursor": ""}
        ) as client:
            provider = KalshiProvider(settings(historical_latency_seconds=0), client)
            rows = await provider.historical_markets(START, START + timedelta(hours=1))
            assert all(
                row.available_at > row.end_time for row in rows if row.result_yes is not None
            )
        async with client_for({"markets": [market_payload()], "cursor": ""}) as client:
            rows = await KalshiProvider(settings(), client).historical_markets(
    ...
    async def run():
        def response(request):
            ...
            return httpx.Response(
                ...
                json={
                    "orderbook_fp": {
                        "yes_dollars": [["0.4800", "3.50"], ["0.5100", "2.25"]],
                        "no_dollars": [["0.4600", "7.50"]],
                    }
                },
            )
        ...
        async with httpx.AsyncClient(transport=httpx.MockTransport(response)) as client:
            provider = KalshiProvider(
                settings(kalshi_api_key="test", kalshi_private_key_path=path), client
            )
...
def test_crossed_book_rejected(monkeypatch):
    async def run():
        async with client_for(
            {"orderbook_fp": {"yes_dollars": [["0.7", "3"]], "no_dollars": [["0.6", "4"]]}}
        ) as client:
            provider = KalshiProvider(settings(), client)
            monkeypatch.setattr(provider, "_auth_headers", lambda *args: {})
...
def test_retry_429_and_no_retry_401(monkeypatch):
    ...
    async def run():
        ...
        config = settings(pyth_api_key="test", pyth_feed_id=FEED).model_copy(
            update={"request_retries": 2}
        )
        async with httpx.AsyncClient(transport=httpx.MockTransport(response)) as client:
...
def test_databento_history_exact_contract_delay_and_receive_time():
    async def run():
        row = {
            "hd": {"ts_event": START.isoformat()},
            "ts_recv": (START + timedelta(microseconds=100)).isoformat(),
            "price": "4301.500000000",
            "symbol": "GCZ5",
        }
        ...
        async with httpx.AsyncClient(transport=httpx.MockTransport(response)) as client:
            provider = DatabentoComexProvider(
                settings(
                    comex_api_key="test", comex_contract="GCZ5", historical_latency_seconds=0.1
                ),
                client,
            )
...
def test_csv_requires_explicit_available_at_delay_and_contract(tmp_path):
    ...
    async def run():
        provider = CSVComexProvider(
            settings(comex_provider="csv", comex_contract="GCZ5", comex_csv_path=path)
        )
        try:
            point = await provider.latest_price()
...
def test_live_databento_keeps_original_receipt_on_repeated_poll(monkeypatch):
    ...
    monkeypatch.setitem(
        sys.modules,
        "databento",
        SimpleNamespace(Live=Live, TradeMsg=Trade, ErrorMsg=type("Error", (), {})),
    )
    raw = []

    async def run():
        provider = DatabentoComexProvider(
            settings(comex_api_key="test", comex_contract="GCZ5"),
            raw_sink=lambda *args: raw.append(args),
        )
        try:
