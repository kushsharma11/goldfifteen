...
def market(**updates):
    return MarketWindow(**({
        "market_id": "gold-1", "start_time": START, "end_time": END,
        "available_at": START, "target_price": 4300,
    } | updates))


def prices():
    return [
        PricePoint(timestamp=START + timedelta(seconds=second),
                   available_at=START + timedelta(seconds=second),
                   price=4300 + 0.05 * (second // 5 % 3), provider="pyth")
        for second in range(-3600, 901, 5)
    ]


def test_only_official_label_and_no_final_target_leakage():
    final = market(available_at=END + timedelta(seconds=20), target_price=9999,
                   settled_price=1, result_yes=True, status="settled", settlement_time=END)
...
def test_late_downloaded_historical_ticks_are_unavailable():
    delayed = [point.model_copy(update={"available_at": END + timedelta(hours=1)}) for point in prices()]
...
def test_retrospective_availability_assumptions_are_exposed_in_dataset_metadata():
    source_metadata = {
        "availability_basis": "assumed_historical_latency", "historical_latency_seconds": 2.0,
    ...
    assert frame.attrs["availability_assumptions"] == [{
        "provider": "pyth", "availability_basis": "assumed_historical_latency",
