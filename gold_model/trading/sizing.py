...
class SizingConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if any(not isfinite(v) or v < 0 for v in (self.fixed_dollar_risk, self.max_position_dollars)):
            raise ValueError("Dollar limits must be finite and nonnegative")
        if not 0 <= self.kelly_fraction <= 1 or not 0 <= self.max_fraction_of_bankroll_per_market <= 1:
...
def size_position(
    ...
    if conservative_cost >= 1:
        return PositionSize(0, 0.0)
    cap = min(bankroll, config.max_position_dollars, bankroll * config.max_fraction_of_bankroll_per_market)
    if config.method == "fixed_contracts":
        ...
        if config.method == "fixed_dollar_risk":
            ...
            kelly = max(0.0, (signal.selected_probability - conservative_cost) / (1 - conservative_cost))
    ...
    if available_depth is not None:
        contracts = min(contracts, max(0, floor(available_depth))) if isfinite(available_depth) else 0
