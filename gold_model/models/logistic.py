...
def validate_dataset(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    for column in DATE_COLUMNS:
        if result[column].isna().any() or any(
            pd.Timestamp(t).tzinfo is None for t in result[column]
        ):
    ...
    if (result["timestamp"] < result["market_start"]).any() or (
        result["timestamp"] >= result["market_end"]
    ).any():
...
def _purge_before(frame: pd.DataFrame, cutoff: pd.Timestamp) -> pd.DataFrame:
    groups = frame.groupby("market_id").agg(
        last_label=("label_available_at", "max"), last_snapshot=("timestamp", "max")
    )
...
def chronological_split(
    frame: pd.DataFrame, train_fraction: float = 0.6, calibration_fraction: float = 0.2
) -> DatasetSplit:
    ...
    return DatasetSplit(
        train.copy(), cal.copy(), test.copy(), tuple(sorted(set(data.market_id) - retained))
    )
...
class LogisticModel:
    ...
    def _features(self, frame: pd.DataFrame) -> np.ndarray:
        ...
        values = (
            data[self.feature_names].apply(pd.to_numeric, errors="coerce").to_numpy(dtype=float)
        )
        if not np.isfinite(values).all():
            invalid = [
                name
                for name, valid in zip(
                    self.feature_names, np.isfinite(values).all(axis=0), strict=True
                )
                if not valid
            ]
            raise ValueError(
                f"Required model features unavailable: {invalid}; no zero substitution is allowed"
            )
        return values
    ...
    def fit(self, train: pd.DataFrame, calibration: pd.DataFrame) -> "LogisticModel":
        ...
        self.estimator.fit(
            self.scaler.transform(x_train), train.label.to_numpy(dtype=int), sample_weight=weights
        )
        raw_cal = self.estimator.predict_proba(self.scaler.transform(x_cal))[:, 1]
        self.calibrator.fit(
            raw_cal, calibration.label.to_numpy(dtype=int), market_weights(calibration)
        )
        ...
        self.metadata = {
            "model_type": "logistic_baseline_feature",
            ...
            "calibration_markets": int(calibration.market_id.nunique()),
            ...
            "validation_metrics": probability_metrics(
                ...
                self.calibrator.predict(raw_cal),
                calibration.market_id.to_numpy(),
            ),
    ...
    def load(cls, path: str | Path, *, trusted: bool = False) -> "LogisticModel":
        if not trusted:
            raise ValueError(
                "Joblib can execute code. Pass trusted=True only for an artifact you trust."
            )
...
def train_logistic(frame: pd.DataFrame, **kwargs) -> tuple[LogisticModel, DatasetSplit]:
    ...
    model.metadata["test_evaluation"] = (
        "Reserved; not evaluated during training. Use evaluate/backtest explicitly."
    )
