            ...
            reliability.append(
                {
                    ...
                    "markets": int(len(set(ids[mask]))),
                }
            )
    return {
        "brier_score": float(brier_score_loss(y, p, sample_weight=weights)),
        "log_loss": float(
            log_loss(y, np.clip(p, 1e-6, 1 - 1e-6), labels=[0, 1], sample_weight=weights)
        ),
...
def compare_probabilities(frame: pd.DataFrame, model_name: str = "logistic") -> dict:
    ...
    for name, column in columns.items():
        if column not in frame:
            ...
            continue
        mask = pd.to_numeric(frame[column], errors="coerce").between(0, 1) & frame.label.isin(
            [0, 1]
        )
        if not mask.any():
    ...
    if available and common.any():
        ...
        results["common_support"] = {
            name: probability_metrics(rows.label, rows[column], rows.market_id)
        ...
        if "market" in shared:
            ...
            for name, values in shared.items():
                ...
                differences[name] = {
                    "brier_difference_vs_market": brier_delta,
                    "log_loss_difference_vs_market": loss_delta,
                    "assessment": assessment,
                }
            results["relative_to_market"] = differences
    results["interpretation"] = (
        "Compare models on common_support. No outperformance claim or threshold optimization is inferred. Market midpoint is a probability benchmark, not a fill price."
    )
    return results
...
def _summary(trades: pd.DataFrame) -> dict:
    ...
    return {
        ...
        "roi": float(trades.pnl.sum() / spent) if spent else None,
    }


def trading_metrics(
    trades: pd.DataFrame, decisions: pd.DataFrame, equity: list[dict], initial_bankroll: float
) -> dict:
    result = _summary(trades)
    values = np.array(
        [initial_bankroll] + [row["settlement_equity"] for row in equity], dtype=float
    )
    ...
    result.update(
        {
            ...
            "pass_markets": int(decisions.market_id.nunique() - trades.market_id.nunique())
            if not trades.empty
            else int(decisions.market_id.nunique()),
            "max_drawdown": float((peaks - values).max()),
            "max_drawdown_fraction": float(
                np.max(np.divide(peaks - values, peaks, out=np.zeros_like(peaks), where=peaks > 0))
            ),
    ...
    data["edge_bucket"] = pd.cut(
        data.estimated_edge,
        [-np.inf, 0.02, 0.05, 0.10, 0.15, np.inf],
        labels=["0-2%", "2-5%", "5-10%", "10-15%", "15%+"],
        right=False,
    )
    data["seconds_remaining_bucket"] = pd.cut(
        data.seconds_remaining,
        [0, 60, 180, 300, 600, np.inf],
        labels=["0-60", "61-180", "181-300", "301-600", "601+"],
        include_lowest=True,
    )
    for column in (
        "edge_bucket",
        ...
        result["breakdowns"][column] = {
            str(key): _summary(group)
            for key, group in data.groupby(column, observed=True, dropna=False)
        }
    return result
