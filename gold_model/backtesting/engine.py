...
def run_backtest(frame: pd.DataFrame, config: BacktestConfig | None = None) -> BacktestResult:
    ...
    if "trained_through" in data:
        if data.trained_through.isna().any() or any(
            pd.Timestamp(value).tzinfo is None for value in data.trained_through
        ):
            raise ValueError(
                "trained_through must contain explicit timezone-aware availability cutoffs"
            )
            ...
            equity.append(
                {
                    ...
                    "settlement_equity": settled_equity,
                    "available_cash": cash,
                }
            )
    ...
    for row in data.to_dict("records"):
        ...
        if vol is not None and len(previous_volatility) >= 10:
            low, high = np.quantile(previous_volatility, [0.33, 0.67])
        ...
        position = size_position(
            ...
            config.costs,
            config.sizing,
            yes_depth if signal.action == "YES" else no_depth,
        )
        ...
        decisions.append(
            {
                ...
                if action == "PASS" and signal.action == "PASS"
                else "Exposure, cash, or depth limit"
                if action == "PASS"
                else signal.reason,
    ...
    metrics.update(
        {
            "mode": config.mode,
            "fees_included": config.costs.include_fees,
            "fee_per_contract": config.costs.fee_per_contract,
            "quadratic_fee_rate": config.costs.quadratic_fee_rate,
            "fee_rounding": "Up to nearest cent per order",
            "slippage_cents": config.costs.slippage_cents,
            "min_edge": config.min_edge,
            "probability_comparison": compare_probabilities(
                data, frame.attrs.get("selected_model_name", "logistic")
            ),
            "execution_assumption": "Top-of-book ask and displayed size; no queue position or latency guarantee"
            if config.mode == "executable"
            else "IDEALIZED midpoint fills without historical depth; not executable profitability",
    ...
    return BacktestResult(
        ...
        decisions_frame,
        metrics,
        pd.DataFrame(equity, columns=["timestamp", "settlement_equity", "available_cash"]),
    )
