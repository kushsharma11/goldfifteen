...
class KalshiProvider(HTTPProvider):
    ...
    def _auth_headers(self, path: str, required: bool = False) -> dict[str, str]:
        ...
        if not key_id or not key_path:
            raise ProviderError("Kalshi order books require KALSHI_API_KEY and KALSHI_PRIVATE_KEY_PATH")
        if not hasattr(self, "_private_key"):
            try:
                self._private_key = serialization.load_pem_private_key(Path(key_path).read_bytes(), password=None)
            except (OSError, ValueError, TypeError):
                raise ProviderError("Unable to read Kalshi private key; use an unencrypted RSA or Ed25519 PEM file") from None
    ...
    async def _get(self, path: str, *, kind: str, required_auth: bool = False, **kwargs: Any) -> tuple[Any, datetime]:
        return await self.get_json(self._url(path), kind=kind, headers_factory=lambda: self._auth_headers(path, required_auth), **kwargs)

    def _normalize_market(self, row: dict[str, Any], received_at: datetime, *, historical: bool) -> MarketWindow:
        try:
            start = utc(row["open_time"])
            end = utc(row["close_time"])
            market_id = str(row["ticker"])
        except (KeyError, ValueError, TypeError):
            raise ProviderError("Kalshi market is missing ticker or aware open/close timestamps") from None
    ...
    def _historical_versions(self, market: MarketWindow, received_at: datetime) -> list[MarketWindow]:
        ...
        opening = market.model_copy(update={
            "available_at": available_at, "status": "open", "result_yes": None,
            "settled_price": None, "settlement_time": None, "raw": raw,
        })
        return [opening, market]

    async def _pages(self, path: str, params: dict[str, Any], *, historical: bool) -> list[MarketWindow]:
        markets = []
        seen_cursors: set[str] = set()
        while True:
            payload, received_at = await self._get(path, kind="historical_markets" if historical else "markets", params=params)
            ...
            if cursor in seen_cursors:
                raise ProviderError("Kalshi repeated a pagination cursor; stopped to avoid an infinite collection loop")
            seen_cursors.add(cursor)
            params = {**params, "cursor": cursor}

    async def active_markets(self) -> list[MarketWindow]:
        markets = await self._pages("/markets", {"series_ticker": self.settings.kalshi_series_ticker, "status": "open", "limit": 1000}, historical=False)
        now = datetime.now(UTC)
        return sorted((market for market in markets if market.start_time <= now < market.end_time), key=lambda market: market.end_time)
    ...
    async def historical_markets(self, start: datetime, end: datetime) -> list[MarketWindow]:
        ...
        recent = await self._pages("/markets", {**common, "min_close_ts": int(start.timestamp()), "max_close_ts": int(end.timestamp())}, historical=True)
        archived = await self._pages("/historical/markets", common, historical=True)
        unique = {(market.market_id, market.available_at, market.status): market for market in archived + recent if start <= market.end_time < end}
    ...
    async def orderbook(self, market_id: str) -> BookSnapshot:
        ...
        return BookSnapshot(
            timestamp=received_at, available_at=received_at, market_id=market_id,
            ...
            yes_bids=yes, no_bids=no, raw=payload,
        )

    async def recent_trades(self, market_id: str, limit: int = 100) -> list[dict[str, Any]]:
        payload, _ = await self._get("/markets/trades", kind="trades", params={"ticker": market_id, "limit": min(max(limit, 1), 1000)})
