...
class Collector:
    ...
    def provider(self, name: str) -> Any:
        if name not in self.providers:
            ...
            cls = {"pyth": PythProvider, "kalshi": KalshiProvider, "comex": DatabentoComexProvider}[
                name
            ]
        ...
        async def collect(name: str) -> tuple[str, dict]:
            try:
                ...
                if name != "kalshi":
                    ...
                    max_age = (
                        self.settings.spot_max_age_seconds
                        if name == "pyth"
                        else self.settings.comex_max_age_seconds
                    )
                ...
                if active_ids != self._active_ids:
                    logger.info(
                        "market_transition", extra={"context": {"active": sorted(active_ids)}}
                    )
                ...
                now = utc_now()
                if (
                    self._last_settlement_refresh is None
                    or (now - self._last_settlement_refresh).total_seconds() >= 60
                ):
                    for old in sorted(
                        self.database.pending_markets(now), key=lambda m: m.end_time, reverse=True
                    )[:10]:
                        try:
                            ...
                            logger.warning(
                                ...
                                extra={"context": {"market_id": old.market_id, "error": str(exc)}},
                            )
                    self._last_settlement_refresh = now
                return name, {"markets": len(markets), "books": books, "errors": failures}
            except Exception as exc:
                logger.error(
                    "provider_failure", extra={"context": {"provider": name, "error": str(exc)}}
                )
    ...
    async def backfill(
        self, source: str, start: datetime, end: datetime, step_seconds: int = 5
    ) -> dict:
        if end <= start:
        ...
        if source == "kalshi":
            ...
            if source == "pyth":
                observations = await provider.historical_prices(
                    start, end, step_seconds=step_seconds
                )
            else:
                observations = await provider.historical_prices(start, end)
            count = sum(
                self.database.add_price(point, "spot" if source == "pyth" else "comex")
                for point in observations
            )
        return {
            "source": source,
            "observations": count,
            "availability": "receipt_time"
            if self.settings.historical_latency_seconds is None
